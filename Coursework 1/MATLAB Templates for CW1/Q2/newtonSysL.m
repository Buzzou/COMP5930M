function [ x,f ] = newtonSysL( fnon, fjac, x0, tol, maxIt, beta, k, G )
...
x = x0;             % initial point
f = feval(fnon,x, beta, k, G);  % initial function values
normf = norm(f);
it = 0;
fprintf(' %d %12.6f\n',it,normf);
...
while (normf>tol) && (it<maxIt) 
  ...
  J = feval( fjac, n, x, f, fnon, beta, k, G ); % build Jacobian
  delta = -J\f;                     % solve linear system
  
  xkp = x;  

  [ x,f,normf ] = lineSearch( fnon, x, normf, delta, beta, k, G ); % update x
